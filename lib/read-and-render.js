...
class ReadAndRender {
	...
	run(page, multiples, format, next) {
		...
		// Special code path for slimerjs...
		if (next) {
			log.trace("slimerjs output");
			if (svgHolder.dims.unit !== "pt") {
				throw new Error("Units of 'pt' are required for png or pdf");
			}
			// use px for screenshot purpose
			svgHolder.dims.unit = "px";
			var writeStream = new streamBuffers.WritableStreamBuffer();
			svgHolder.finalize(writeStream);
			var svgBuffer = writeStream.getContents();
			rasterize.slimerjs(svgBuffer, svgHolder.dims.width, svgHolder.dims.height, 1, svgHolder.numPages, format, null, next);
			return;
		}
		...
		if (format === "svg") {
			...
			let rasterBuffer = rasterize.rsvg(svgBuffer, svgHolder.dims.width, svgHolder.dims.height, svgHolder.numPages, format);
			return rasterBuffer;
		} else {
