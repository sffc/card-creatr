...
const {stripXML, applyXML} = require("./phrasing-content");
...
class CardRenderer {
	...
	_wrapText(text, options) {
		const {newText, strippedParts} = stripXML(text);
		let wrappedText = options.wrappr.wrap(newText, options.fontSize, options.width);
		return applyXML(wrappedText, strippedParts);
	}

	_makeTextPath(line, options) {
		line = new String(line);  // in case a non-string argument like a number is passed
		line = stripXML(line).newText; // strip xml before width computations
	...
	render(cardOptions, globalOptions, viewport, extraOptions) {
		var locals = {
			...
			_wrapText: this._wrapText.bind(this),
