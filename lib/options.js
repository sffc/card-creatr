 ...
 *
 * serializeFieldKey: The inverse of parseFieldKey.
 *
		...
		if (shouldLoadBuffer) {
			...
			try {
				...
				if (field.properties.img) {
					buffer = fs.readFileSync(path.join(__dirname, "placeholder.png"));
					...
					"buffer": (__next) => {
						...
						if (shouldLoadBuffer) {
							...
							fn(result.path, (err, buffer) => {
								...
								// If file is not found, and the field has the "img" property, fail silently and load the placeholder image instead.
								if (field.properties.img) {
									fs.readFile(path.join(__dirname, "placeholder.png"), __next);
									result.mimeType = mime.lookup("placeholder.png");
								} else {
									return __next(err);
								}
