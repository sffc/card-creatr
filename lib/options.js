 ...
 *
 * serializeFieldKey: The inverse of parseFieldKey.  Accepts one extra property, "array", to reconstruct the full field key.
 *
...
function processFieldSync(field, oldValue, dirname) {
	...
	if (Object.keys(field.properties).length === 0) {
		...
		if (shouldLoadBuffer) {
			...
			try {
				...
				if (field.properties.img) {
					buffer = fs.readFileSync(path.join(__dirname, "placeholder.jpg"));
				} else {
					throw err;
				}
			}
		...
		// Save the data in the result object only if "path" is requested
				...
				async.auto({
					"buffer": (__next) => {
						...
						if (shouldLoadBuffer) {
							...
							fn(result.path, (err, buffer) => {
								if (!err) return __next(null, buffer);
								// If file is not found, and the field has the "img" property, fail silently and load the placeholder image instead.
								if (field.properties.img) {
									fs.readFile(path.join(__dirname, "placeholder.jpg"), __next);
								} else {
									return __next(err);
								}
							});
...
Options.serializeFieldKey = serializeFieldKey;
